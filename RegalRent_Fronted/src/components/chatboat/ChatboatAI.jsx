...
const speakMessage = (text) => {
  ...
  return (
    <>
     ...
     <div className="cbp-btn-unique" onClick={() => setOpen(prev => !prev)}>
  {mode === "ai" ? <SmartToyIcon /> :
   mode === "team" ? <SupportAgentIcon /> :
   <ChatIcon />}
</div>
      ...
      {open && (
        <div className="cbp-box-unique">
          ...
          <div className="cbp-header-unique">
            ...
            <span className="cbp-title-unique">
  {mode === "ai" && <> <SmartToyIcon /> AI Assistant </>}
  {mode === "team" && <> <SupportAgentIcon /> Chat with Us </>}
  {!mode && <> <ChatIcon /> Start Chat </>}
</span>
...
<div className={`voice-option ${voiceType === "female" ? "active" : ""}`} onClick={() => setVoiceType("female")}>
  <WomanIcon />
</div>
          <div
  className={`speak-toggle ${isSpeaking ? "active" : ""}`}
