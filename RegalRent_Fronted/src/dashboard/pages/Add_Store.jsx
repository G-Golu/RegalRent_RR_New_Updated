...
// //  all ok , comment for add auto email send after add stor 25-03-2026
...
//     alert("Password required for new store");
//     return;
//   }
...
//     if (editId) {
//       res = await updateStore(editId, fd);
//       alert(res?.data?.message || "Store updated successfully");
//     } else {
//       res = await createStore(fd);
//       alert(res?.data?.message || "Store created successfully");
//     }
...
//  all ok , 26-03-2026 , comment for add remainder email
...
//     const storesArray = res.data.stores || [];

//     const normalized = storesArray.map((store) => ({
//       ...store,
//       categories: Array.isArray(store.categories)
//         ? store.categories
...
//     // Optional: sort by id to keep cards in order
...
// all ok auto remainder email send successfully email send 26-03-2026
...
{list.map((u) => {
  const pkg = packageList.find((p) => p.id === Number(u.package_id));
  const pkgName = pkg?.package_name || u.package_name || "Package";
...
let daysLeft = null;
let daysClass = ""; // this will be applied to <span>

if (!pkg || pkg.status === 0) {
  finalStatus = "Disabled";
} else if (pkg && u.created_at) {
  const createdDate = new Date(u.created_at);
  const expiryDate = new Date(createdDate);
  expiryDate.setDate(createdDate.getDate() + Number(pkg.days));
  ...
  if (totalDaysLeft <= 0) {
    finalStatus = "Expired";
    daysLeft = 0;
    daysClass = "store-days-expired"; // red
  } else if (totalDaysLeft <= 7) {
    daysLeft = totalDaysLeft;

    if (totalDaysLeft <= 3) {
      daysClass = "store-days-critical"; // red for last 3 days
    } else {
      daysClass = "store-days-warning"; // orange for 7 → 4 days
    }
  } else {
    // More than 7 days → green
    daysClass = "store-days-active";
  }
}
...
<p>
  Status: {finalStatus}{" "}
  {daysLeft !== null && (
    <span className={daysClass}>{`(${daysLeft} days left)`}</span>
  )}
</p>

  // Determine package type (for badge color)
  let packageType = "silver";
  if (pkg) {
    const days = Number(pkg.days);
    if (days <= 28) packageType = "silver";
    else if (days <= 180) packageType = "gold";
    else if (days <= 365) packageType = "platinum";
    else packageType = "platinum-pro";
  }

  // Status color
  let statusColor = "";
  if (finalStatus === "Active") statusColor = "green";
  else if (finalStatus === "Expired") statusColor = "red";
  ...
  return (
    <div className="profile-card temple-card" key={u.id}>
      ...
      <img
        ...
        src={u.logo ? `${BACKEND_URL}${u.logo}` : defaultLogo}
        alt="logo"
        style={{ borderColor: getPackageColor(pkg?.package_name) }}
      />

      <div className="store-name">Name: {u.name}</div>

      <div className="store-package">
        Package: {pkgName} ({finalStatus})
      </div>
      ...
      >
        Status: {finalStatus}
        {finalStatus === "Active" && daysLeft !== null
          ? ` (${daysLeft} days left)`
          : ""}
      </div>

      <div className="store-category">
        Category: {Array.isArray(u.categories)
          ? u.categories.join(", ")
          : JSON.parse(u.categories || "[]").join(", ")}
      </div>
