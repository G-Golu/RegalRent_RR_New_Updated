...
// // //Now this is final and fixed correct code 100% ok , done.

// import { useLocation, useNavigate } from "react-router-dom";
// import { useState, useEffect } from "react";
// import { createCheckoutApi } from "../../api/shopAdmin/Checkout/checkoutApi.js";
...
//       const staffArray = res.data.data || res.data.staff || res.data || [];
...
//     return sum + Number(item.deposit_amount || 0);
...
//   const advanceAmount =
//     Number(customerData.advance_amount || 0);

//   const dueAmount =
//     Math.max(grandTotal - advanceAmount, 0);
...
//     if (name === "advance_amount" && Number(value) < 0)
//       return;
...
//       return alert("Select ID Type");

//     if (!customerData.id_number.trim())
//       return alert("Enter ID Number");
...
//       return alert("Advance cannot exceed total");
...
//     // ✅ SAFE DATE FORMAT (NO Date object)
//     const rows = products.map((item) => ({
//       product_id: item.product_id || item.id,
//       product_name: item.name,

//       // direct string split (safe)
...
//     console.log("SENDING TO BACKEND:", rows); // 🔍 DEBUG
...
//     navigate("/shop-admin/my-order-page");
...
//           <div className="product-day-list">
...
//               const total =
//                 item.rent_amount * days;

//               return (

//                 <div
//                   key={item.id}
//                   className="product-day-item"
//                 >
...
//                     {priceMode === "day"
//                       ? `:  Rs. ${item.rent_amount} × ${days} = Rs. ${total}`
//                       : `:  Fixed Price : = Rs. ${item.rent_amount}`
//                     }
...
// all good now comment for add payment 
...
// this is correct payment -- now for some time for comment today is : 28-02-2026
...
//         body: JSON.stringify({ amount: grandTotal, checkoutData: buildCheckoutData() })
//       });
//       const session = await response.json();
//       if (!session.url) return alert("Payment session failed");
...
//   // ================= PLACE ORDER =================
//   const handlePlaceOrder = async () => {
//     if (!isPaid) return alert("Please complete payment first!");
//     if (!validateForm()) return;
...
//             {products.map((item) => {
//               const days = calculateDays(item.start_date, item.end_date);
//               const total = item.rent_amount * days;
//               return (
//                 <div key={item.id} className="product-day-item">
//                   <span>{item.name}</span>
//                   <span>
//                     {priceMode === "day"
//                       ? `:  Rs. ${item.rent_amount} × ${days} = Rs. ${total}`
...
//           type="button"
//           className="place-order-btn"
//           onClick={isPaid ? handlePlaceOrder : handlePayment}
//           disabled={loading}
//         >
//           {loading ? (isPaid ? "Saving..." : "Processing Payment...") : (isPaid ? "Save Order" : "Pay Now")}
//         </button>
//       </div>
//     </div>
//   );
